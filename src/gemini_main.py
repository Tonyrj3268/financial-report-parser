...
from gemini_transform import parse_with_file, parse_with_markdown
...
def process(pdf_path, prompt, model, target_pages=None):
    ...
    if not fonts_missing_tounicode(pdf_path):
        ...
        reply = parse_with_file(pdf_path, prompt, model)
    else:
...
def main():
    ...
    for filename in pdf_mapping.keys():
        res = process_wrapper(filename, modelname)
        if res[1]:
            results[filename] = res[1]
        else:
            failed[filename] = res[2]
